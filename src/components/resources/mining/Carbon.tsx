...
const Carbon: React.FC = () => {
  ...
  useEffect(() => {
    fetch('/exp.csv')
      ...
      .then(data => {
        ...
        let filteredData = parsedData.filter(item => Object.values(item).some(value => value.includes('CARBON')))
  ...
  return (
    ...
    <><IonCard>
        <img alt='Carbon' src='CARBON.webp' />
        <IonCardHeader>
          <IonCardTitle>
            <a href='https://play.staratlas.com/market/CARBWKWvxEuMcq3MqCxYfi7UoFVpL9c4rsQS99tw6i4X/'>Carbon</a>
        </IonCardTitle>
      </IonCardHeader>
      <IonCardContent>
      {buyData.length > 0 ? (
